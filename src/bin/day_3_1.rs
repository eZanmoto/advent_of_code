...
fn run() -> Result<Option<i32>, Error> {
    let mut file = File::open("inputs/day_3_1.txt")?;
    ...
    Ok(closest_intersection(paths[0], paths[1]))
}
...
fn pts_to_lines(pts: Vec<Pt>) -> Vec<Ln> {
    (0..pts.len()-1)
...
impl Ln {
    fn new(src: Pt, tgt: Pt) -> Ln {
        Ln{src: src, tgt: tgt}
    }

    fn man_poi(a: &Ln, b: &Ln) -> Option<Pt> {
        let (vert, horz) =
            if a.src.x == a.tgt.x {
                (a, b)
            } else {
